...
async function initializeDatabase() {
    try {
        ...
        const apiCount = await db.get('SELECT COUNT(*) as count FROM available_apis');
        if (apiCount.count === 0) {
            const defaultApis = [
            ...
            for (const api of defaultApis) {
                await db.run(
                    'INSERT INTO available_apis (name, display_name, endpoint, required_params) VALUES (?, ?, ?, ?)',
                    [api.name, api.display_name, api.endpoint, api.required_params]
                );
            }
        ...
        if (userCount.count === 0) {
            ...
            await db.run(
                ...
                ['sahil', sahilHash, 'head_admin']
            );
            await db.run(
                'INSERT INTO users (username, password, role) VALUES (?, ?, ?)',
                ['superadmin', superadminHash, 'admin']
            );
...
app.get('/', async (req, res) => {
    try {
        ...
        const apis = await db.all('SELECT * FROM available_apis');
...
app.get('/endpoints', async (req, res) => {
    try {
        if (!db) throw new Error('Database not initialized');
        const apis = await db.all('SELECT * FROM available_apis WHERE is_active = 1');
...
app.get('/head-admin/dashboard', requireHeadAdmin, async (req, res) => {
    try {
        if (!db) throw new Error('Database not initialized');
        const users = await db.all('SELECT * FROM users WHERE role != ?', ['head_admin']);
...
app.post('/admin/generate-key', requireAuth, async (req, res) => {
    try {
        ...
        async function createKey(apiKey, isCustom) {
            let expires_at = null;
            const now = new Date();
        ...
        if (isCustomEnabled && custom_key && custom_key.trim() !== '') {
            ...
            const existing = await db.get('SELECT * FROM api_keys WHERE key = ?', [apiKey]);
            if (existing) {
                return res.status(400).send('❌ Key already exists: ' + apiKey);
            }
            await createKey(apiKey, true);
        } else {
            let apiKey = 'OSINT_' + Math.random().toString(36).substring(2, 18).toUpperCase();
            await createKey(apiKey, false);
        }
    } catch (error) {
...
app.post('/admin/toggle-status', requireAuth, async (req, res) => {
    try {
        ...
        await db.run('UPDATE api_keys SET status = ? WHERE id = ?', [newStatus, id]);
        res.redirect('/admin/dashboard');
    } catch (error) {
...
app.post('/head-admin/remove-admin', requireHeadAdmin, async (req, res) => {
    try {
        ...
        await db.run('DELETE FROM users WHERE id = ? AND role != ?', [admin_id, 'head_admin']);
        res.json({ success: true });
    } catch (error) {
...
app.post('/reset-all-passwords', async (req, res) => {
    try {
        ...
        await db.run(
            ...
            [1, 'sahil', sahilHash, 'head_admin']
        );
        
        const superadminHash = await bcrypt.hash('sexy', 10);
        await db.run(
            'INSERT OR REPLACE INTO users (id, username, password, role) VALUES (?, ?, ?, ?)',
            [2, 'superadmin', superadminHash, 'admin']
        );
...
app.all('/api/:endpoint', globalLimiter, async (req, res) => {
    ...
    try {
        ...
        const keyData = await db.get(
            'SELECT * FROM api_keys WHERE key = ? AND status = ?',
            [userKey, 'active']
        );
        ...
        if (keyData.expires_at && new Date(keyData.expires_at) < new Date()) {
            await db.run('UPDATE api_keys SET status = ? WHERE id = ?', ['expired', keyData.id]);
            return res.json({ error: 'Key expired', contact: OWNER });
        }
        
        await db.run('UPDATE api_keys SET hits = hits + 1 WHERE id = ?', [keyData.id]);
...
cron.schedule('0 0 * * *', async () => {
    try {
        if (!db) {
            console.log('⚠️ Database not initialized, skipping cron job');
            return;
        }
        const now = new Date().toISOString();
        await db.run(
            'UPDATE api_keys SET status = ? WHERE expires_at IS NOT NULL AND expires_at < ? AND status != ?',
            ['expired', now, 'expired']
        );

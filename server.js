...
// ========== MASTER API HANDLER - DIRECT RESULT WAHI JO SOURCE SE AATA HAI ==========
async function handleMasterAPI(query) {
    try {
        // Master API source - direct result without any modification
        ...
        // Return original data - NO CLEANING, NO MODIFICATION
        return response.data;
    } catch (error) {
...
// ========== RESPONSE CLEANER (SIRF DEVELOPER AUR CHANNEL REMOVE KAREGA, BAKI SAB RAKHEGA) ==========
function cleanResponseData(data, endpoint = null) {
    if (!data || typeof data !== 'object') {
        return data || {};
    }

    // MASTER API KE LIYE - DIRECT ORIGINAL DATA BINA KUCH CHANGE KE
    if (endpoint === 'master') {
        // Sirf developer aur channel fields remove karo
        const cleaned = JSON.parse(JSON.stringify(data));
        
        const fieldsToRemove = [
            'developer', 'Developer', 'dev', 'Dev',
            'channel', 'Channel', 'telegram', 'Telegram',
            'contact', 'Contact', 'credit', 'Credit',
            'owner_username', 'owner_channel', 'created_by',
            '@bmw_aura5', 'bmw_aura5', 'OSINTNXERA'
        ];
        
        function removeFields(obj) {
            if (!obj || typeof obj !== 'object') return;
            for (let key in obj) {
                if (fieldsToRemove.includes(key) || fieldsToRemove.includes(key.toLowerCase())) {
                    delete obj[key];
    ...
    // BAKI APIs KE LIYE - Normal cleaning
    try {
...
app.all('/api/:endpoint', globalLimiter, async (req, res) => {
    ...
    db.get('SELECT * FROM api_keys WHERE key = ? AND status = "active"', [userKey], async (err, keyData) => {
        ...
        // MASTER API - DIRECT RESULT WITHOUT MODIFICATION
        if (endpoint === 'master') {
            ...
            // Sirf developer aur channel fields remove karo, baaki original rakho
...
app.listen(PORT, () => {
    ...
    console.log('   - Direct result from source API');
    console.log('   - Sirf developer aur channel fields remove honge');

...
// ========== COMPLETE ENDPOINT MAPPING (SAB ENDPOINTS ADD KAR DIYE) ==========
const ENDPOINT_ALIASES = {
    ...
    // SAB APIS - HAR EK ENDPOINT KA MAPPING
    'aadhar': 'aadhar_info',
...
db.serialize(() => {
    ...
    db.get(`SELECT COUNT(*) as count FROM available_apis`, [], (err, row) => {
        if (row && row.count === 0) {
            const apis = [
                ['master', '🔧 Master API', 'master', 'query', '{"query":"search"}', 'Master API endpoint', '1'],
...
// ========== COMPLETE API PROXY MAP (SAB ENDPOINTS ADD KAR DIYE) ==========
const apiProxyMap = {
    'master': null, // Master API handled separately
...
async function handleMasterAPI(query) {
    try {
        ...
        console.log(`🔗 Master API calling: ${masterApiUrl}`);
        ...
        return { 
            error: 'Master API request failed', 
...
// ========== RESPONSE CLEANER - HATANA HAI CHANNEL AUR DEVELOPER ==========
function cleanResponseData(data, endpoint = null) {
    ...
    try {
        ...
        // YEH SAB FIELDS HATANE HAIN (SPECIALLY CHANNEL AUR DEVELOPER)
        const removeFields = [
            // CHANNEL AUR DEVELOPER WALE
            'CHANNEL', 'Channel', 'channel', 
            'DEVELOPER', 'Developer', 'developer', 'Dev', 'dev',
            '📢 CHANNEL', '👨‍💻 DEVELOPER',
            'none_usrX1', 'None_usernam3',
            'https://t.me/none_usrX1', '@None_usernam3',
            
            // BAKI JO PEHLE SE THE
            'DM TO BUY ACCESS', 'owner', 'xtradeep', 'Kon_Hu_Mai', 
            ...
            'method', 'query_time_ms', 'resolved_id', 'noobgamingv40',
            'bronx_tg_key', 'BRONXop', 'RACK2', 'RACKSUN', 'email_info', 'vehicle_owner_number',
            'techvishalboss', 'TVB_FULL', 'lookup.php', 'status', 'message',
        ...
        function cleanObject(obj) {
            ...
            for (let key in obj) {
                if (obj.hasOwnProperty(key)) {
                    ...
                    // Check string values for channel/developer
                    else if (typeof obj[key] === 'string') {
                        if (obj[key].includes('t.me/none_usrX1') || 
                            obj[key].includes('None_usernam3') ||
                            (obj[key].includes('@') && !obj[key].includes('bmw_aura5'))) {
                            delete obj[key];
                        }
                        if (obj[key].includes('techvishalboss') || obj[key].includes('TVB_FULL') || 
                            obj[key].includes('InvalidAyush') || obj[key].includes('AMMOL_ZZ') || 
                            obj[key].includes('CYBERXANMOL') || obj[key].includes('ftgamer2') ||
        ...
        // Sirf apna channel aur owner add karo
...
app.all('/api/:endpoint', globalLimiter, async (req, res) => {
    ...
    db.get('SELECT * FROM api_keys WHERE key = ? AND status = "active"', [userKey], async (err, keyData) => {
        ...
        // Get the mapped endpoint name
        const mappedEndpoint = ENDPOINT_ALIASES[endpoint] || endpoint;
        const proxyFn = apiProxyMap[mappedEndpoint];

        if (!proxyFn) {
            return res.json({ error: 'Unknown endpoint', contact: '@bmw_aura5', endpoint: endpoint });
        }
        ...
        try {
            ...
            } else if (mappedEndpoint === 'mistral') {
...
app.listen(PORT, () => {
    ...
    console.log('✅ All 27 Endpoints Working Now');
    console.log('✅ Channel & Developer Fields Removed From Response');
    console.log('=====================================');

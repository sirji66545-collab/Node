...
db.serialize(() => {
    ...
    db.run(`ALTER TABLE api_keys ADD COLUMN is_custom INTEGER DEFAULT 0`, () => {});
    db.run(`ALTER TABLE api_keys ADD COLUMN rate_limit_enabled INTEGER DEFAULT 1`, () => {});
    db.run(`ALTER TABLE api_keys ADD COLUMN rate_limit_per_day INTEGER DEFAULT 100`, () => {});
    db.run(`ALTER TABLE api_keys ADD COLUMN rate_limit_per_hour INTEGER DEFAULT 20`, () => {});
    db.run(`ALTER TABLE api_keys ADD COLUMN rate_limit_per_minute INTEGER DEFAULT 5`, () => {});
...
app.post('/admin/generate-key', requireAuth, (req, res) => {
    ...
    console.log('📝 Generate key request:', { name, enable_custom, custom_key, expiry });
    ...
    function createKey(apiKey, isCustom) {
        ...
        console.log('💾 Creating key:', { apiKey, isCustom, expires_at });
    ...
    if (enable_custom === 'true' || enable_custom === true) {
        ...
        if (apiKey.length < 4) {
            return res.status(400).send('Custom key must be at least 4 characters');
        }
...
app.listen(PORT, () => {
    ...
    console.log('✅ Custom keys working now!');
    console.log('=====================================\n');
});

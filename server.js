...
app.post('/admin/delete-key', requireAuth, (req, res) => {
    try {
        ...
        const index = DB.api_keys.findIndex(k => k.id === parseInt(id));
        if (index === -1) {
...
app.post('/admin/toggle-status', requireAuth, (req, res) => {
    try {
        ...
        if (key) {
            key.status = status === 'active' ? 'disabled' : 'active';
        }
        res.redirect('/admin/dashboard');
    } catch (error) {
...
app.post('/head-admin/update-rate-limit', requireHeadAdmin, (req, res) => {
    try {
        ...
        if (key) {
            ...
            key.unlimited_hits = isUnlimited ? 1 : 0;
            key.rate_limit_enabled = isUnlimited ? 0 : (rate_limit_enabled === 'true' ? 1 : 0);
            key.rate_limit_per_day = parseInt(rate_limit_per_day) || 100;
            key.rate_limit_per_hour = parseInt(rate_limit_per_hour) || 20;
            key.rate_limit_per_minute = parseInt(rate_limit_per_minute) || 5;
        }
...
app.post('/head-admin/remove-admin', requireHeadAdmin, (req, res) => {
    try {
        ...
        const index = DB.users.findIndex(u => u.id === parseInt(admin_id) && u.role !== 'head_admin');
        if (index !== -1) {
...
app.post('/reset-all-passwords', async (req, res) => {
    try {
        ...
        let sahil = DB.users.find(u => u.username === 'sahil');
        if (sahil) {
...
app.all('/api/:endpoint', globalLimiter, async (req, res) => {
    ...
    try {
        const keyData = DB.api_keys.find(k => k.key === userKey && k.status === 'active');

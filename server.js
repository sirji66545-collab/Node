    ...
    db.get(`SELECT COUNT(*) as count FROM available_apis`, [], (err, row) => {
        if (row && row.count === 0) {
            const apis = [
                ...
                ['tg_to_number', '📞 TG to Number', 'tg-to-number', 'username', '{"username":"@username"}', 'Get mobile number from Telegram username', '1'],
                ...
                ['snapchat', '👻 Snapchat Profile', 'snapchat', 'username', '{"username":"username"}', 'Snapchat profile', '1'],
...
// ========== RESPONSE CLEANER - COMPLETELY REMOVE CHANNEL & DEVELOPER ==========
function cleanResponseData(data, endpoint = null) {
    ...
    try {
        ...
        const removeFields = [
            ...
            'none_usrX1', 'None_usernam3', 'None_usrX1', 'none_usernam3',
            'https://t.me/none_usrX1', 't.me/none_usrX1',
            '@None_usernam3', 'None_usernam3',
            'telegram_channel', 'tg_channel', 'channel_link',
            'developer_name', 'dev_name', 'created_by',
            'owner', 'Owner', 'OWNER'
        ];

        // Remove fields from entire object recursively
        function removeFieldsDeep(obj) {
            ...
            for (let key of Object.keys(obj)) {
                ...
                if (removeFields.includes(key) || 
                    removeFields.includes(key.toLowerCase()) ||
                ...
                if (typeof obj[key] === 'string') {
                    // Remove if string contains channel or developer info
                    if (obj[key].includes('t.me/none_usrX1') || 
                        obj[key].includes('/none_usrX1') ||
                        obj[key].includes('@None_usernam3') ||
                        obj[key].includes('None_usernam3') ||
                        obj[key].includes('none_usrX1') ||
                        obj[key].includes('None_usrX1') ||
                        (obj[key].includes('t.me/') && obj[key].includes('none')) ||
                        (obj[key].includes('@') && obj[key].includes('None_'))) {
                        delete obj[key];
                        continue;
                    }
                    
                    // Remove any @ mentions except our own
                    if (obj[key].includes('@') && 
                        !obj[key].includes('bmw_aura5') && 
                        !obj[key].includes('OSINTNXERA')) {
                        delete obj[key];
                        continue;
                    }
                }
        ...
        // If cleaned is array, clean each item
        if (Array.isArray(cleaned)) {
            cleaned = cleaned.map(item => {
                if (typeof item === 'object' && item !== null) {
                    removeFieldsDeep(item);
                }
                return item;
            });
        }
        
        // Add our own owner and channel
...
app.listen(PORT, () => {
    ...
    console.log('✅ Channel & Developer Fields Removed Completely');
    console.log('=====================================');
